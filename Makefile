...
_use-local-library:
	@for game in $(GAMES); do \
		if [ -d "$(GAMES_DIR)/$$game/chart" ]; then \
			sed -i 's|repository: "oci://harbor.7kgroup.org/7kminato/charts"|repository: "file://../../../charts/_library"|' $(GAMES_DIR)/$$game/chart/Chart.yaml; \
...
_use-oci-library:
	@for game in $(GAMES); do \
		if [ -d "$(GAMES_DIR)/$$game/chart" ]; then \
			sed -i 's|repository: "file://../../../charts/_library"|repository: "oci://harbor.7kgroup.org/7kminato/charts"|' $(GAMES_DIR)/$$game/chart/Chart.yaml; \
			rm -f $(GAMES_DIR)/$$game/chart/Chart.lock $(GAMES_DIR)/$$game/chart/charts/minato-games-library-*.tgz; \
		fi \
	done
